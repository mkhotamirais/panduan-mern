...
export const postUser = createAsyncThunk("users/postUser", async (data, { rejectWithValue }) => {
  try {
    const response = await axios.post(`${mys}/v4/user`, data);
...
export const deleteUser = createAsyncThunk("users/deleteUser", async (data, { rejectWithValue }) => {
  try {
    const response = await axios.delete(`${mys}/v4/user/${data?.uuid}`);
    return response.data;
  } catch (error) {
    return rejectWithValue(error?.response?.data?.message || error?.message);
  }
...
export const updateUser = createAsyncThunk("users/updateUser", async (data, { rejectWithValue }) => {
  try {
    const response = await axios.patch(`${mys}/v4/user/${data?.uuid}`, data);
    return response.data;
  } catch (error) {
    return rejectWithValue(error?.response?.data?.message || error?.message);
  }
...
const mysV4UserSlice = createSlice({
  name: "mysV4User",
  initialState: {
    data: [],
    status: "idle",
    error: null,
    view: JSON.parse(localStorage.getItem("mysV4UserView")) || "table",
    sort: "createdAt",
  },
  reducers: {
...
export const { setSort, setView } = mysV4UserSlice.actions;

export default mysV4UserSlice.reducer;
