...
export const getProducts = createAsyncThunk("products/getProducts", async (_, { rejectWithValue }) => {
  try {
    const response = await axios.get(`${mys}/v2/product`);
...
export const postProduct = createAsyncThunk("products/postProduct", async (data, { rejectWithValue }) => {
  try {
    const response = await axios.post(`${mys}/v2/product`, data);
...
export const deleteProduct = createAsyncThunk("products/deleteProduct", async (data, { rejectWithValue }) => {
  try {
    const response = await axios.delete(`${mys}/v2/product/${data?.id}`);
    return response.data;
  } catch (error) {
    return rejectWithValue(error?.response?.data?.message || error?.message);
  }
...
export const updateProduct = createAsyncThunk("products/updateProduct", async (data, { rejectWithValue }) => {
  try {
    const response = await axios.patch(`${mys}/v2/product/${data.id}`, data);
    return response.data;
  } catch (error) {
    return rejectWithValue(error?.response?.data?.message || error?.message);
  }
...
const mysV2ProductSlice = createSlice({
  name: "mysV2Product",
  initialState: {
    data: [],
    status: "idle",
    error: null,
    view: JSON.parse(localStorage.getItem("mysV2ProductView")) || "table",
    sort: "createdAt",
  },
  reducers: {
...
export const { setSort, setView } = mysV2ProductSlice.actions;

export default mysV2ProductSlice.reducer;
