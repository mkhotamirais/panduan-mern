...
import { getProducts, setSort, setView } from "../../../../app/features/mongodb/mdV3ProductSlice";
import { Err, GridCard, Loading, PostBtn } from "../../../../components/Components";
import MdV3ProductCard from "./MdV3ProductCard";
import MdV3ProductTable from "./MdV3ProductTable";

const MdV3Product = () => {
  const dispatch = useDispatch();
  const { data: products, status, error, sort, view } = useSelector((state) => state.mdV3Product);
  ...
  if (status === "loading") content = <Loading />;
    ...
    if (products.length > 0) {
      const renderedProductsCard = products && sortedData?.map((item) => <MdV3ProductCard key={item?._id} item={item} />);
      const renderedProductsTable =
        products && sortedData?.map((item, i) => <MdV3ProductTable key={item?._id} item={item} i={i} />);
      if (view === "card") {
        ...
        <div className="flex gap-1">
          <select
            ...
            onChange={(e) => {
              ...
              localStorage.setItem("msfProductView", JSON.stringify(e.target.value));
            }}
