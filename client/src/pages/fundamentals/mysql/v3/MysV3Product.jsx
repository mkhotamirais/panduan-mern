...
import { getProducts, setSort, setView } from "../../../../app/features/mysql/mysV3ProductSlice";
import { Err, GridCard, Loading, PostBtn } from "../../../../components/Components";
import MysV3ProductCard from "./MysV3ProductCard";
import MysV3ProductTable from "./MysV3ProductTable";

const MysV3Product = () => {
  const dispatch = useDispatch();
  const { data: products, status, error, sort, view } = useSelector((state) => state.mysV3Product);
  ...
  if (status === "loading") content = <Loading />;
    ...
    if (products.length > 0) {
      const renderedProductsCard = products && sortedData?.map((item) => <MysV3ProductCard key={item?.id} item={item} />);
      const renderedProductsTable =
        products && sortedData?.map((item, i) => <MysV3ProductTable key={item?.id} item={item} i={i} />);
      if (view === "card") {
        ...
        <div className="flex gap-1">
          <select
            ...
            onChange={(e) => {
              ...
              localStorage.setItem("mysV3ProductView", JSON.stringify(e.target.value));
            }}
