...
import { getProducts, setSort, updateProduct } from "../../../../app/features/mysql/mysV4ProductSlice";
import { Button, Input, InputRef, Label, Select } from "../../../../components/Tags";
import { PiSpinner } from "react-icons/pi";
import { getUsers } from "../../../../app/features/mysql/mysV4UserSlice";

const MysV4ProductUpdate = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { id } = useParams();
  const item = useSelector((state) => state.mysV4Product.data.find((s) => s.id.toString() === id));

  const { data: users } = useSelector((state) => state.mysV4User);
  const { status } = useSelector((state) => state.mysV4Product);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (canSave) {
      const data = { uuid: item?.uuid, name, price, userId };
...
export default MysV4ProductUpdate;
