...
import { getProducts, setSort, setView } from "../../../../app/features/mysql/mysV1ProductSlice";
import { Err, GridCard, Loading, PostBtn } from "../../../../components/Components";
import MysV1ProductCard from "./MysV1ProductCard";
import MysV1ProductTable from "./MysV1ProductTable";

const MysV1Product = () => {
  const dispatch = useDispatch();
  const { data: products, status, error, sort, view } = useSelector((state) => state.mysV1Product);
  ...
  if (status === "loading") content = <Loading />;
    ...
    if (products.length > 0) {
      const renderedProductsCard = products && sortedData?.map((item) => <MysV1ProductCard key={item?.id} item={item} />);
      const renderedProductsTable =
        products && sortedData?.map((item, i) => <MysV1ProductTable key={item?.id} item={item} i={i} />);
      if (view === "card") {
        ...
        <div className="flex gap-1">
          <select
            ...
            onChange={(e) => {
              ...
              localStorage.setItem("mysV1ProductView", JSON.stringify(e.target.value));
            }}
