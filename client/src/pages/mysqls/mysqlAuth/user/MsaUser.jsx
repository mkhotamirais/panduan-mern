...
import { getUsers, setSort, setView } from "../../../../app/features/mysqlAuth/msaUserSlice";
import { Err, GridCard, Loading, PostBtn } from "../../../../components/Components";
import MsaUserCard from "./MsaUserCard";
import MsaUserTable from "./MsaUserTable";
import useRefresh from "../auth/useRefresh";

const MsaUser = () => {
  const dispatch = useDispatch();
  const { data: users, status, error, sort, view } = useSelector((state) => state.msaUser);
  const { axiosJWT, token } = useRefresh();

  useEffect(() => {
    dispatch(getUsers({ axiosJWT, token }));
  }, [dispatch, token]);
  ...
  if (status === "loading") content = <Loading />;
    ...
    if (users.length > 0) {
      const renderedProductsCard = users && sortedData?.map((item) => <MsaUserCard key={item.id} item={item} />);
      const renderedProductsTable = users && sortedData?.map((item, i) => <MsaUserTable key={item.id} item={item} i={i} />);
      if (view === "card") {
