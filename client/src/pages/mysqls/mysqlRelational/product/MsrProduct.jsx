...
import { getProducts, setSort, setView } from "../../../../app/features/mysqlRelational/msrProductSlice";
import { Err, GridCard, Loading, PostBtn } from "../../../../components/Components";
import MsrProductTable from "./MsrProductTable";
import MsrProductCard from "./MsrProductCard";

const MsrProduct = () => {
  const { data: products, status, error, view, sort } = useSelector((state) => state.msrProduct);
  ...
  if (status === "loading") content = <Loading />;
    ...
    if (products.length > 0) {
      const renderedProductsCard = products && sortedData?.map((item) => <MsrProductCard key={item.id} item={item} />);
      const renderedProductsTable =
        products && sortedData?.map((item, i) => <MsrProductTable key={item.id} item={item} i={i} />);
      if (view === "card") {
